...
import { daysBetween, formatShortDate, today } from "./utils/date";
  ...
  return (
    <section className="stack">
      ...
      <div className="stat-grid dashboard-stats">
        ...
        <Stat label="Top damaged area" value={stats.topAreas[0]?.label || "None"} />
        <Stat label="Top damage type" value={stats.mostCommonReason} />
      </div>
      ...
      <div className="split">
        <div className="panel">
          <h2>Top Damaged Areas</h2>
          <MiniList items={stats.topAreas.slice(0, 5)} empty="No area data yet." />
        </div>
        <div className="panel">
          <div className="section-title">
  ...
  return (
    <section className="panel form-panel">
      ...
      <div className="form-grid">
        ...
        <Select label="Area" value={form.area} options={["", ...settings.areas]} onChange={(v) => set("area", v)} />
...
function EntryList({
  ...
  const [filters, setFilters] = useState({ priority: "", reason: "", area: "", aisle: "", location: "", person: "", date: "" });
  const filtered = entries.filter((entry) =>
    (!filters.priority || entry.priority === filters.priority) &&
    (!filters.reason || entry.reason === filters.reason) &&
    (!filters.area || entry.area === filters.area) &&
    ...
    <section className="stack">
      <div className="panel">
        ...
        <div className="filter-grid">
          ...
          <Select compact label="Area" value={filters.area} options={["", ...settings.areas]} onChange={(v) => setFilters({ ...filters, area: v })} />
        ...
        <button className="ghost small" onClick={() => setFilters({ priority: "", reason: "", area: "", aisle: "", location: "", person: "", date: "" })}>
          Clear Filters
        </button>
      </div>
      <div className="cards">
        {filtered.map((entry) => (
          <EntryCard
            ...
            onMarkReplaced={openMode ? () => {
              ...
              onSave({ ...entry, conditionStatus: "Replaced", dateRepaired: entry.dateRepaired || today(), repairedBy, repairAction, reviewFlag: "Complete" });
            } : undefined}
  ...
  return (
    <article className={`entry-card priority-${entry.priority.toLowerCase()}`}>
      ...
      <dl>
        ...
        <div><dt>Area</dt><dd>{entry.area || "Not set"}</dd></div>
        <div><dt>Aisle</dt><dd>{entry.aisle || "Not set"}</dd></div>
        <div><dt>Review</dt><dd>{entry.reviewFlag || (entry.dateRepaired ? "Complete" : "Needs repair")}</dd></div>
        ...
        <div><dt>Days</dt><dd>{daysBetween(entry.dateReported, entry.dateRepaired) || "Open"}</dd></div>
      </dl>
...
function AnalyticsPage({ stats }: { stats: ReturnType<typeof buildAnalytics>; entries: CantileverEntry[] }) {
  return (
    <section className="stack">
      ...
      <div className="split">
        <Panel title="Top Damaged Areas"><MiniList items={stats.topAreas} empty="No area data yet." /></Panel>
  ...
  return (
    <section className="stack">
      ...
      <div className="split">
        ...
        <SettingsList title="Area List" value={settings.areas} onChange={(v) => updateList("areas", v)} />
